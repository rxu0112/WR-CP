...
def generate_triplets(n):
    triplets = []
    for _ in range(n):
        ai = random.uniform(0.6, 1)  # give more weight to one domain to be more distinct with P_{XY}
...
endpoint = np.array([[0, 50], [50, 100], [100, 150], [150, 200], [200, 250], [250, 299], [299, 347]])

train_envs, cal_envs, test_envs_d = [], [], []
Years = np.array([0, 1, 2, 3, 4])
for index, loc in enumerate(locations):
    Input_stack = preprocess(raw_data, endpoint, loc)
    Input_year = [Input_stack[i[0]:i[1], :] for i in endpoint]
    np.random.shuffle(Years)
    train_data = np.concatenate((Input_stack[endpoint[Years[0]][0]:endpoint[Years[0]][1], :],
                                 Input_stack[endpoint[Years[1]][0]:endpoint[Years[1]][1], :]))
    cal_data = np.concatenate((Input_stack[endpoint[Years[2]][0]:endpoint[Years[2]][1], :],
                               Input_stack[endpoint[Years[3]][0]:endpoint[Years[3]][1], :],
                               Input_stack[endpoint[Years[4]][0]:endpoint[Years[4]][1], :]))
    test_data = Input_stack[250:,:]
...
kde_params = {'bandwidth': np.logspace(-2, 0.5, 20)}
...
pickle.dump(train_envs, open(project_path + "/data/japan/processed/train_v" + str(ver), "wb"))
pickle.dump(test_envs_q, open(project_path + "/data/japan/processed/test_v" + str(ver), "wb"))
np.save(project_path + '/data/japan/processed/cal_uni_v' + str(ver) + '.npy', cal_envs_uni)