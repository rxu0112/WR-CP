...
def get_hours(date, roads):
    ...
    fil_path = os.path.join(project_path, "data\\pemsd4\\raw\\pemsd4.npz")
    raw_data = np.load(fil_path)['data']  # (time*node*(volume,occupy,speed)
    date_range = pd.date_range(start='2018-01-01 00:00:00', periods=16992, freq='5T')
...
roads = [[69, 72], [254, 257], [104, 107], [49, 52], [86, 89], [92, 95], [162, 165], [29, 32], [35, 38], [31, 34]]
project_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
workday_path = os.path.join(project_path, "data\\pemsd4\\raw\\Workday.txt")
...
pickle.dump(train_envs, open(project_path + "/data/pemsd4/processed/train_v" + str(ver), "wb"))
pickle.dump(test_envs_q, open(project_path + "/data/pemsd4/processed/test_v" + str(ver), "wb"))
np.save(project_path + '/data/pemsd4/processed/cal_uni_v' + str(ver) + '.npy', cal_envs_uni)
